        ...
        if section == "Version Information":
            ...
            if not calculated_fields_df.empty:
                ...
                if not df_data_sources.empty:
                    logger.info(f"Data Sources Columns Before Merge: {df_data_sources.columns.tolist()}")
                    logger.info(f"Calculated Fields Columns Before Merge: {calculated_fields_df.columns.tolist()}")
                    ...
                    logger.info(f"Calculated Fields Columns After Merge: {calculated_fields_df.columns.tolist()}")
                    logger.info(f"Sample Data After Merge:\n{calculated_fields_df.head()}")
                    ...
                    if 'Data Source Caption' not in calculated_fields_df.columns:
                        logger.error("'Data Source Caption' column is missing after merging.")
                        raise KeyError("'Data Source Caption' column is missing after merging.")
            ...
            if not worksheets_df.empty:
                ...
                if not df_data_sources.empty:
                    ...
                    worksheets_df.rename(columns={'Caption': 'Data Source Caption'}, inplace=True)
...
def convert_html_to_pdf(source_html):
    try:
        ...
        # Configure logging
        logzero.logfile(os.path.join(log_dir, "report.log"), maxBytes=1e6, backupCount=3)
