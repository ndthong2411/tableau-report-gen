...
def generate_dag(
    ...
    if selected_worksheet and selected_worksheet != "All":
        ...
        worksheet_columns = worksheets_df[worksheets_df['Worksheet Name']
                                          == selected_worksheet]['Column Name'].tolist()
        # Also include calculated fields used in this worksheet
        calculated_in_ws = calculated_fields_df[calculated_fields_df['Dependencies'].apply(
            lambda deps: any(dep in worksheet_columns for dep in deps))]['Field Name'].tolist()
    else:
